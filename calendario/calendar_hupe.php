...
<?php
	...
	<div class="rodape2">
		<table align="center">
		<tr>
			<th><a class="linksmenuativo" href="calendar_hupe.php?ano=2020" >2020</a></th>
			<th>|</th>
			<th><a class="linksmenuativo" href="calendar_hupe.php?ano=2019" >2019</a></th>
		</tr>
		</table>
	</div>
	<?php
			...
			for ($spalte=1; $spalte<=4; $spalte++) {
				...
				while ($i<=$insgesamt) {
					...
					if (($i==$day) && ($this_month==$month)) { //---> hoje
						...
						$sql_hupe = "select id_hupe, ano, boletim, dt_boletim from hupe where ano='$year' and dt_boletim='$dt_boletim'";
						$rs_hupe = mysqli_query($con,$sql_hupe) or die(mysql_error());
						
						//echo $sql_hupe;						
						
						if(mysqli_num_rows($rs_hupe)<>0){
							while ($linha_hupe=mysqli_fetch_array($rs_hupe)) {
															
								$id_hupe=$linha_hupe['id_hupe'];
								$ano=$linha_hupe['ano'];
								$boletim=$linha_hupe['boletim'];
								$dt_boletim=$linha_hupe['dt_boletim'];

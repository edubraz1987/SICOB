...
<?php
	...
	<div class="rodape2">
		<table align="center"> 
		<tr>
			<th><a class="linksmenuativo" href="calendar_convenio.php?ano=2020" >2020</a></th>
			<th>|</th>
			<th><a class="linksmenuativo" href="calendar_convenio.php?ano=2019" >2019</a></th>
		</tr>
		</table>
	</div>
	<?php
			...
			for ($spalte=1; $spalte<=4; $spalte++) {
				...
				while ($i<=$insgesamt) {
					...
					if (($i==$day) && ($this_month==$month)) { //---> hoje
						...
						$sql_convenio = "select id_convenio, ano, boletim, dt_boletim from convenio where ano='$year' and dt_boletim='$dt_boletim'";
						$rs_convenio = mysqli_query($con,$sql_convenio) or die(mysql_error());
						
						//echo $sql_convenio;						
						
						if(mysqli_num_rows($rs_convenio)<>0){
							while ($linha_convenio=mysqli_fetch_array($rs_convenio)) {
															
								$id_convenio=$linha_convenio['id_convenio'];
								$ano=$linha_convenio['ano'];
								$boletim=$linha_convenio['boletim'];
								$dt_boletim=$linha_convenio['dt_boletim'];
								...
								echo '<span style="color:#bbb;"><a class="linque" title="BOLETIM '.$zeros_boletim.'" target="_blank" href="exibe_boletim.php?boletim='.$boletim.'&data='.$dia.'-'.$mes.'-'.$ano.'">'.$i.'<a></span>'; //aqui entra o IF do boletim com o link do banco	
								//echo '<span style="color:#bbb">'.$i.'</span>';
							}
						
						}
						else{
							echo '<span style="color:#bbb;">'.$i.'<a></span>';
						}
					} 

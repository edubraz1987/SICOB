...
<?php
	...
	<div class="rodape2">
		<table align="center">
		<tr>
			<th><a class="linksmenuativo" href="calendar_financeiro.php?ano=2020" >2020</a></th>
			<th>|</th>
			<th><a class="linksmenuativo" href="calendar_financeiro.php?ano=2019" >2019</a></th>
		</tr>
		</table>
	</div>
	<?php
	...
	$_SESSION['tipo'] = "financeiro";
	$_SESSION['sistema']  = "bol";
	
	//echo("<script type='text/javascript'> alert('Feliz $year!');</script>");
			...
			for ($spalte=1; $spalte<=4; $spalte++) {
				...
				while ($i<=$insgesamt) {
					...
					if (($i==$day) && ($this_month==$month)) { //---> hoje
						...
						$sql_financeiro = "select id_financeiro, ano, boletim, dt_boletim from financeiro where ano='$year' and dt_boletim='$dt_boletim'";
						$rs_financeiro = mysqli_query($con,$sql_financeiro) or die(mysql_error());
						
						//echo $sql_financeiro;						
						
						if(mysqli_num_rows($rs_financeiro)<>0){
							while ($linha_financeiro=mysqli_fetch_array($rs_financeiro)) {
															
								$id_financeiro=$linha_financeiro['id_financeiro'];
								$ano=$linha_financeiro['ano'];
								$boletim=$linha_financeiro['boletim'];
								$dt_boletim=$linha_financeiro['dt_boletim'];
